...
interface GallerySettingsProps {
  ...
  expanded: boolean;
  onToggle: () => void;
  onChange: (section: 'gallery', field: string, value: unknown) => void;
  projects: Project[] | undefined;
}

export function GallerySettings({ settings, expanded, onToggle, onChange, projects }: GallerySettingsProps) {
  const activeProjects = useMemo(() => {
    return projects?.filter(p => p.status === 'active') || [];
  }, [projects]);

  const selectedProjects = settings.content?.gallery?.projects || [];

  const handleAddProject = (projectId: string) => {
    if (!projectId) return;
    if (!selectedProjects.includes(projectId)) {
      onChange('gallery', 'projects', [...selectedProjects, projectId]);
    }
  };

  const handleRemoveProject = (projectId: string) => {
    onChange('gallery', 'projects', selectedProjects.filter(id => id !== projectId));
  };
